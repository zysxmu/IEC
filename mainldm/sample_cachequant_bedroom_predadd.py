...
if __name__ == "__main__":
    ...
    if batch_group > 1:
        ...
        hooks = []
        hooks.append(
            AttentionMap_add(model.model.diffusion_model.output_blocks[-2], interval_seq=range(args.ddim_steps),
                             end_t=args.ddim_steps))

        logging.info("sample predadd start!")
        for ii in range(batch_group):

            with torch.no_grad():
                _, intermediates = sampler.sample(args.ddim_steps, batch_size=bs, shape=shape, eta=args.ddim_eta,
                                                  verbose=False, )

            feature_maps = hooks[0].out
            feature_maps_list.extend(feature_maps)

        torch.save(feature_maps, "./calibration/bedroom_feature_maps_interval{}.pt".format(args.replicate_interval))
